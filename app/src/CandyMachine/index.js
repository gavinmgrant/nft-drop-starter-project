...
const CandyMachine = ({ walletAddress }) => {
  ...
  const [isBeforeDrop, setIsBeforeDrop] = useState(true);
    ...
    if (data.length !== 0) {
      for (const mint of data) {
        ...
        if (!mints.find((mint) => mint.image === parse.image)) {
          setMints((prevState) => [
            ...prevState,
            { image: parse.image, name: parse.name },
          ]);
        }
      }
    }
  ...
  const renderMintedItems = () => (
    <div className="gif-container">
      <p className="sub-text">Minted NFTs</p>
  ...
  useEffect(() => {
    ...
    currentDate < dropDate ? setIsBeforeDrop(true) : setIsBeforeDrop(false);
  }, [machineStats]);
      ...
      <div className="machine-container">
        ...
        {machineStats.itemsRedeemed === machineStats.itemsAvailable ? (
          ...
          <button
            ...
            onClick={mintToken}
            disabled={isMinting || isBeforeDrop}
          >
            {isMinting ? "Minting..." : "Mint NFT"}{isBeforeDrop && " (Coming Soon)"}
          </button>
        )}
